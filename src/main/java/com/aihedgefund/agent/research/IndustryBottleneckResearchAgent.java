 ...
 * 产业瓶颈反向拆解研究 Agent。
 *
 * <p>根据用户输入的一句话行业/标的描述，使用「行业瓶颈反向拆解选股法」五步框架，
 ...
 */
@Component
public class IndustryBottleneckResearchAgent {

    private static final Logger log = LoggerFactory.getLogger(IndustryBottleneckResearchAgent.class);

    /** 系统提示词文件名（对应 prompts/industry_bottleneck_research.md） */
    private static final String PROMPT_NAME = "industry_bottleneck_research";
    ...
    private static final ScheduledExecutorService HEARTBEAT_SCHEDULER = Executors.newScheduledThreadPool(2, r -> {
        Thread t = new Thread(r, "industry-research-heartbeat");
        t.setDaemon(true);
        return t;
    });
    ...
    public IndustryBottleneckResearchAgent(LlmClientFactory llmFactory, PromptLoader promptLoader) {
    ...
    /**
     * 对一句话产业研究问题执行五步瓶颈反向拆解分析。
     *
     * @param query         用户输入的一句话产业/标的描述及筛选条件
    ...
    public String analyze(String query, String modelName, String modelProvider, Consumer<String> activityCallback) {
        ...
        log.debug("[industry-bottleneck-research] 可用工具 {} 个: {}", allCallbacks.length,
        ...
        log.info("[industry-bottleneck-research] 开始分析, query={}, model={}, 工具数={}",
                query, resolvedModelName, allCallbacks.length);

        pushActivity(activityCallback, "开始产业瓶颈反向拆解分析…");
        ...
        log.debug("[industry-bottleneck-research] LLM 最终响应: {}", report);
        return report;
    }
