...
import com.aihedgefund.agent.research.ContrarianSectorResearchAgent;
...
import com.aihedgefund.model.req.ContrarianAnalysisReq;
...
/**
 * 热门行业逆向对立面分析接口：用户输入一句话热门行业/分析需求，使用「通用热门行业逆向对立面
 * 价值标的挖掘」五步框架进行研究分析，以 SSE 流式返回分析进度和最终 Markdown 报告。
 */
@RestController
@RequestMapping("/contrarian-analysis")
public class ContrarianAnalysisController {

    private static final Logger log = LoggerFactory.getLogger(ContrarianAnalysisController.class);

    private final ContrarianSectorResearchAgent researchAgent;
    ...
    public ContrarianAnalysisController(ContrarianSectorResearchAgent researchAgent,
    ...
    public SseEmitter run(@RequestBody @Valid ContrarianAnalysisReq req) {
        ...
        Long runId = analysisRunService.createRunningContrarianAnalysis(
                currentUser.getUserId(), req.getQuery(), req.getModelName());

        log.info("收到逆向对立面分析请求, query={}, userId={}, runId={}, SSE 超时={}ms",
        ...
        Executors.newSingleThreadExecutor().submit(() -> {
            try {
                ...
                analysisRunService.markCompleteContrarianAnalysis(runId, report);
                ...
                log.error("逆向对立面分析失败: {}", e.getMessage(), e);
                analysisRunService.markError(runId, e.getMessage());
                try {
